...
<!-- 로그인 확인에 대한 코드 삽입(주의사항 html 태그 중복 제거 -->
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<script type="text/javascript">
	function validateForm(form) {//폼 내용 검증 - validateForm 을 호출하면 form의 매개 값을 받는다
		if (form.title.value == "") { //폼안에 제목의 값을 확인하여 빈칸이면
			alert("제목을 입력하세요"); //경고창과 메세지를 출력
			form.title.focus(); //제목 박스에 커서를 이동한다
		...
		if (form.content.value == "") { //폼안에 내용의 값을 확인하여 빈칸이면
			alert("내용을 입력하세요");
			//경고창과 메세지를 출력
			form.content.focus(); //제목 박스에 커서를 이동한다
			return false; //결과로 실패를 보냄

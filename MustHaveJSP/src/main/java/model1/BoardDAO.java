...
public class BoardDAO extends JDBConnect { // 3번째 생성자로 활용
	...
	public BoardDAO(ServletContext app) {
		// ServletContext app -> WEB-INF \ web.xml 에 있는 값 활용
		super(app);

	}// con,stmt,pstmt,rs,close 를 제공 받는다
	...
	public int selectCount(Map<String, Object> map) {
		...
		if (map.get("searchword") != null) {
			query += "WHERE " + map.get("searchField") + "" // key
					+ "LIKE '%" + map.get("searchword") + "%'"; // value
	...
	public List<BoardDTO> selectList(Map<String, Object> map) {
		...
		String query = "SELECT * FROM board "; //조건이 없을때
		if (map.get("searchWord") != null) {
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map) {
		...
		if (map.get("searchWord") != null) {
			query += " WHERE " + map.get("searchField") + " " + " LIKE '%" + map.get("searchWord") + "%' ";//조건잇을때 where 추가 if문
		...
		try {
			...
			pstmt.setString(1,map.get("start").toString()); // 문자 변환
			pstmt.setString(2,map.get("end").toString()); 
			
			rs=pstmt.executeQuery(); //쿼리 결과 실행
			...
			System.out.println("게시물 조회 중 예외 발생 selectListPage()를 확인하세요");
			e.printStackTrace();
		}
	...
	public int insertWrite(BoardDTO dto) {
		...
		try {
			...
			pstmt = con.prepareStatement(query); // 동적쿼리
			...
			System.out.println("계시물 입력 중 예외 발생 (insertwrite메서드를 확인하세요");
			e.printStackTrace();
		}
		return result;
	}
	...
	public BoardDTO selectView(String num) {
		...
		try {
			...
			//결과처리
			if(rs.next()) {
				dto.setNum(rs.getString("num")); //테이블 순서가 안맞으면 손으로 쓰기
	...
	public int updateEdit(BoardDTO dto) {
		...
		try {
			...
			result = pstmt.executeUpdate(); //쿼리문 실행하여 결과를 int 로 받음
		} catch (Exception e) {
	...
	public int deletePost(BoardDTO dto) {
		int result =0;
		try {
			String query = "DELETE FROM board WEHRE num=?";
